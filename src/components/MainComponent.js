  ...
  componentDidMount() {
    ...
    .then(response => {
      ...
      if(localStorage.getItem('ipAddress') && localStorage.getItem('ipAddress') === response.ip) {
        ...
        .then(res => res.json())
        .then(res => {
          //Unless API Call Limit Exceeded 
          if(!res.message){
      ...
      <div>
        ...
        <div class="container-fluid">
          <Switch>
            ...
            <Route path="/currency" component={() => <Currency city={this.state.city} currency={this.state.currency} />} exact/>
            <Route path="/lifestyle" component={() => <LifeStyle currency={this.state.currency} lifestyle={this.state.lifestyle} city={this.state.city} />} exact/>
